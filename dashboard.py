...
app.layout = html.Div(
    ...
    children=[
        html.H1("IoT Sensor Dashboard", style={'textAlign': 'center', 'color': '#ffffff'}),

        # Threshold Inputs and Buttons
        html.Div([
            html.Div([
                html.Label("Light Threshold:", style={'color': '#ffffff'}),
                dcc.Input(id='light-threshold-input', type='number', value=200, className='input-box'),
                html.Label("Distance Threshold:", style={'color': '#ffffff'}),
                dcc.Input(id='distance-threshold-input', type='number', value=200, className='input-box'),
                html.Label("Temperature Threshold:", style={'color': '#ffffff'}),
            ...
            ], className='card', style={'backgroundColor': '#333333', 'padding': '10px', 'borderRadius': '5px'})
        ...
        html.Div(id='insight-panel', style={'textAlign': 'center', 'fontSize': '16px', 'color': '#66b2ff', 'marginBottom': '20px'}),
        ...
        html.Div(id='uptime-display', style={'textAlign': 'center', 'marginTop': '20px', 'fontSize': '14px', 'color': '#cccccc'}),
...
def update_dashboard(_, light_thresh, distance_thresh, temp_thresh, security_clicks):
    if not temperatures:
        fig = go.Figure()
        fig.update_layout(template="plotly_dark", title="Waiting for Data...")
    ...
    temp_fig.update_layout(template="plotly_dark", title="Temperature (°C)", yaxis_title='°C')
    ...
    ld_fig.update_layout(template="plotly_dark", title="Light & Distance", yaxis_title='Sensor Value')
    ...
    minmax_fig.update_layout(template="plotly_dark", barmode='group', title='Min/Max Sensor Values', height=400)
    ...
    trend_fig.update_layout(template="plotly_dark", title='Sensor Trends (Smoothed)', yaxis=dict(title='Direction', tickvals=[-1, 0, 1], ticktext=['↓', '→', '↑']), height=400)
...
def update_avg_bar(n):
    ...
    fig.update_layout(template="plotly_dark", title="Average Sensor Values (Last 30 seconds)", yaxis_title="Average Value", height=400)
    return fig
...
def update_cumulative_avg(n):
    ...
    fig.update_layout(template="plotly_dark", title="Cumulative Average Sensor Values", yaxis_title="Average Value", height=400)
    return fig
